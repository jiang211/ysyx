...
assign EXU_BTB_updata_valid = (IDU_EXU_PC_sel[4]) && (IDU_EXU_valid && EXU_IDU_ready);
...
assign pc_opdata1 = (IDU_EXU_PC_sel[1] || IDU_EXU_PC_sel[4] || zero) ? pc_data : RS1_data;
...
//wire [4:0] pc_sel = {IDU_EXU_jal || IDU_EXU_B_type, IDU_EXU_ecall ,IDU_EXU_mret, IDU_EXU_jal, IDU_EXU_jalr};
assign EXU_IFU_pc =
            (IDU_EXU_PC_sel[2] || IDU_EXU_PC_sel[3]) ? csr_data      :
            (IDU_EXU_PC_sel[1] || IDU_EXU_PC_sel[0] || zero) ? next_pc   :
                                  pc_data + 32'd4;
...
begin 
   if(rstn)begin
    ...
    EXU_LSU_ecall               <=        IDU_EXU_PC_sel[3];
    EXU_LSU_mret               <=        IDU_EXU_PC_sel[2];
