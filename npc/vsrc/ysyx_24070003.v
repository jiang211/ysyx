...
reg [2 ** INDEX_WIDTH -1:0]btb_valid [WAY_NUM-1:0];          // 有效位
...
assign hit[0] = btb_valid[0][update_index] && (btb_tag[cur_index][0] == cur_tag);
assign hit[1] = btb_valid[1][update_index] && (btb_tag[cur_index][1] == cur_tag);

assign pred_valid = hit[0] || hit[1];

assign update_hit[0] = ~btb_valid[0][update_index] ;
assign update_hit[1] = ~btb_valid[1][update_index] ;
