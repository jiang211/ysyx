...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge  , B, if((int)src1 >= (int)src2){s->dnpc = s->pc + (int)imm;}printf("src1 == %x, src2 == %x, imm == %d , s->pc == %x, s->dnpc == %x, result == %d\n",src1,src2,imm,s->pc,s->dnpc,((int)src1 >= (int)src2)););
