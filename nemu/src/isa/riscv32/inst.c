...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt  , B, if((long int)src1 < (long int)src2){s->dnpc = s->pc + (int)imm;});
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge  , B, if((long int)src1 >= (long int)src2){s->dnpc = s->pc + (int)imm;});
